...
const GemstoneProduct = () => {
  ...
  useEffect(() => {
    ...
    const fetchRelatedProducts = async () => {
      try {
        const response = await fetch("https://gemstore-backend.onrender.com/api/gemstone/all");
        const data = await response.json();

        if (product) {
          const { Color, Type } = product;
          const filteredProducts = data
            .filter(p => p._id !== product._id && p.Color === Color && p.Type === Type)
            .slice(0, 8);
          ...
          {loading ? (
            ...
            <div className="cards">
              {relatedProducts.map((p) => (
                <Link key={p._id} to={`/gemstone/product/${p._id}`}>
                  <div className="card">
                    <div className="img">
